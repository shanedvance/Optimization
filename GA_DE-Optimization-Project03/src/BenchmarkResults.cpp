...
void run()
{
    ...
    bm.setSimulation(constraints.g_max);
    GA ga(bm);
    ...
    // continually loop through the prompts and execution of the programs until the user exit the program
    while (true)
    {
        ...
        {
            compute(fitness.at(i), &bm, ga, constraints, selection, i);
        }
...
{
    // create some output allowing the user to select an option
    cout << "GENETIC ALGORITHMS (GA):" << endl;
    cout << "=============================" << endl;
    cout << "1. Simple GA" << endl;
    cout << "2. Differential Evolution " << "{Strategy=" << strategy << "}" << endl;
 ...
 * @param ga the GeneticAlgorithm (GA) class that contains the GA functions
...
void compute(Benchmark::Fitness f, Benchmark *bm, GA ga, CF cf, int selection, unsigned int i)
{
    ...
    for (int dim = cf.dimension.at(i).LB; dim <= cf.dimension.at(i).UB; dim += cf.dimension.at(i).LB)
    {
        ...
        switch (selection - 1)
        {
            case 0:
            {
                vector<double> tmpSol = ga.simpleGA(f, cf.ns, dim, cf.range.at(i), cf.g_max, cf.cr, cf.mutation, cf.er);
                for (const double &best_fit : tmpSol)
                {
                    temp.emplace_back(toString(best_fit));
                }
                break;
            }
            case 1:
            {
                vector<double> tmpSol = ga.diffEvolution(f, static_cast<unsigned int>(dim), cf.g_max, cf.ns, cf.F,
                        cf.cr, cf.range.at(i), cf.strategy);
                for (const double &best_fit : tmpSol)
                {
                    temp.emplace_back(toString(best_fit));
                }
