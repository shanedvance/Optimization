...
public:
    ...
    vector<double> iterativeLocalSearch(Benchmark::Fitness, const vector<double> &, double *,
                                             double, int, Range);
...
private:
    ...
    vector<double> random_vector(int dim, Range rng)
    {
        ...
        for (int i = 0; i < dim; ++i)
        {
            randNumber = generate_random_number(engine);
            temp.emplace_back(randNumber);
        }
                                     ...
                                     Range rng, Benchmark::Fitness f)
    {
        ...
        for (unsigned int i = 0; i < x.size(); ++i)
        {
            ...
            gradient = (this->bm.*f)(x_temp) - f_x; // calculate the gradient

            /* Gradient descent formula */
            x_loc = x_temp.at(i) - alpha * gradient;

            /* Check if f(x_loc) <= f(x) */
            if (x_loc <= rng.LB || x_loc >= rng.UB)
            {
                x_loc = (x_loc <= rng.LB) ? rng.LB : rng.UB;
            }
