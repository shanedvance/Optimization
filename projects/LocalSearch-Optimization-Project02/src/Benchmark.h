...
private:
        ...
        else
        {
            ...
            for (string header : this->csvHeaders)
            {
                ...
                if (header == "SIMU-")
                {
                    ...
                    {
                        string hd = header;
                        tempChar = (i < this->simulation_count)
                                   ? (hd.append(to_string(i).append(","))).c_str()
            ...
            for (vector<string> vec : this->csvResults)
            {
                ...
                {
                    const char *tempChar = (i != vec.size() - 1) ? (vec.at(i).append(",")).c_str() : vec.at(i).c_str();
                    fputs(tempChar, csvFile); // Put comma-delimited text in CSV file
                }
    ...
    void generate_random_matrix(int dim, int matrixSize, Range rng)
    {
        ...
        if (matrixSize > 0)
        {
            ...
            generate_random_matrix(dim, matrixSize - 1, rng);
