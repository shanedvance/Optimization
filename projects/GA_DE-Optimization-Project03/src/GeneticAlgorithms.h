...
typedef struct Population
{
    ...
    explicit Population(vector<double> genes) : genome(move(genes)) {}
    vector<double> genome;
    double cost{};

    /* For sorting and getting min_element or max_element of the population by cost */
...
class GeneticAlgorithms
{
    ...
    public:
        ...
        vector<double> simpleGA(Benchmark::Fitness, int, unsigned int, Range, int, double, Mutation, double);
    ...
    private:
        ...
        vector<double> selectParent(const vector<Population> &p)
        {
            ...
            return p.at(s).genome; // return the fittest parent
        }

        Parent select(const vector<Population> &p)
        {
            Parent parent;
            parent.one = selectParent(p); // select individual for parent one
            parent.two = selectParent(p); // select individual for parent two
            return parent;
        }
        ...
        double getBestSolution(Benchmark::Fitness f, const vector<Population> &p, bool isSimpleGA)
        {
            if (isSimpleGA)
            {
                ...
                return (this->bm.*f)(x_best->genome);
            }
            else
            {
        ...
        void evaluate(Benchmark::Fitness f, vector<Population> *p)
        {
            ...
            for (Population &pop : *p) 
	    {
                pop.cost = (this->bm.*f)(pop.genome);
            }
            this->totalFitness = this->sum(*p);
        }

        void selectIndexes(unsigned int *r1, unsigned int *r2, unsigned int *r3, unsigned int *r4, unsigned int *r5, int np, int i, int strategy)
        {
                            ...
                            unsigned int i, int np, int strategy)
        {
            ...
            switch (strategy)
            {
                ...
                case 1:
                {
                    ...
                    do
                    {
                        u->at(j) = x_best.at(j) + F * (x.at(r2).genome.at(j) - x.at(r3).genome.at(j));
                        j = (j + 1) % this->dim;
                        ++l;
                    }
                ...
                case 2:
                {
                    ...
                    do
                    {
                        u->at(j) = x.at(r3).genome.at(j) + F * (x.at(r1).genome.at(j) - x.at(r2).genome.at(j));
                        j = (j + 1) % this->dim;
                        ++l;
                    }
                    while(this->random(Range(0.0, 1.0)) < cr && l < this->dim);
                ...
                case 3:
                {
                    ...
                    do
                    {
                        u->at(j) = x.at(i).genome.at(j) + F * (x_best.at(j) - x.at(i).genome.at(j))
                                  + F * (x.at(r1).genome.at(j) - x.at(r2).genome.at(j));
                        j = (j + 1) % this->dim;
                        ++l;
                    }
                    while(this->random(Range(0.0, 1.0)) < cr && l < this->dim);
                ...
                case 4:
                {
                    ...
                    do
                    {
                        u->at(j) = x_best.at(j) + F * (x.at(r1).genome.at(j) + x.at(r2).genome.at(j)
                                                      - x.at(r3).genome.at(j) - x.at(r4).genome.at(j));
                        j = (j + 1) % this->dim;
                        ++l;
                    }
                    while(this->random(Range(0.0, 1.0)) < cr && l < this->dim);
                ...
                case 5:
                {
                    ...
                    do
                    {
                        u->at(j) = x.at(r5).genome.at(j) + F * (x.at(r1).genome.at(j) + x.at(r2).genome.at(j)
                                                               - x.at(r3).genome.at(j) - x.at(r4).genome.at(j));
                        j = (j + 1) % this->dim;
                        ++l;
                    }
