    ...
    for (int t = 0; t < iMax; ++t)
    {
        ...
        for (unsigned int j = 0; j < pMax; ++j)
        {
            ...
            for (unsigned int i = 0; i < dim; ++i)
            {
                ...
                p.at(j).velocity.at(i) = p.at(j).velocity.at(i) + c1 * this->random(Range(0.0, 1.0)) * (pBest.at(j)
                        - p.at(j).particle.at(i)) + c2 * this->random(Range(0.0, 1.0)) * (pBest.at(j)
                                                                                          - p.at(j).particle.at(i));
