...
double GeneticAlgorithms::simpleGA(Benchmark::Fitness f, int ns, unsigned int dim, Range rng, int t_max,
                                                         double cr, Mutation m, double er)
{

    vector<double> bestSolutions;
    double gBest = numeric_limits<double>::max();
    ...
    for (int t = 1; t <= t_max; ++t)
    {
        ...
        double best = this->getBestSolution(f, p, true);
        if (best < gBest)
        {
            gBest = best;
        }
...
double GeneticAlgorithms::diffEvolution(Benchmark::Fitness fn, unsigned int dim, int g_max, int np, double F,
    ...
    if (np >= 4)
    {
        ...
        for (int g = 1; g <= g_max; ++g)
        {
            ...
            for (unsigned int i = 0; i < static_cast<unsigned int>(np); ++i)
            {
                ...
                vector<double> u = x.at(i).particle;
            ...
            double best = this->getBestSolution(fn, x, false);
            if (best < gBest)
            {
                gBest = best;
            }
