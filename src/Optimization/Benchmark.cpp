...
double Benchmark::sine_envelope_sine_wave(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    ...
    result = result + (0.5 + ((pow(sin(value_one + value_two - 0.5), 2.0)) / (1.0 + 0.001 * pow((value_one + value_two), 2.0))));
  }
...
double Benchmark::stretch_v_sine_wave(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    ...
    result = result + (pow(value_one + value_two, four_root) * sin(pow(50.0 * pow(value_one + value_two, ten_root), 2.0)) + 1.0);
  }
...
double Benchmark::ackley_two(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    denominator_one = (pow(M_E, 0.2) * sqrt((pow(x.at(i), 2.0) + pow(x.at(i + 1), 2.0)) / 2.0));

    result = result +
             (20.0 + M_E - (20.0 / denominator_one) - pow(M_E, 0.5 * (cos(2.0 * M_PI * x.at(i)) + cos(2.0 * M_PI * x.at(i + 1)))));
  }
...
double Benchmark::rana(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    ...
    result = result + (x.at(i) * sin(sqrt_one) * cos(sqrt_one) + (x.at(i + 1) + 1.0) * cos(sqrt_one) * sin(sqrt_one));
  }
...
double Benchmark::pathological(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    ...
    result = result + (0.5 + (sin(pow(sqrt_one, 2.0)) - 0.5 / (1.0 + 0.001 * pow(pow(x.at(i), 2.0) - (2.0 * x.at(i)) * x.at(i + 1) + pow(x.at(i + 1), 2.0), 2.0))));
  }
...
double Benchmark::masters_cosine_wave(vector<double> x)
{
  ...
  for (unsigned int i = 0; i < x.size() - 1; ++i)
  {
    value_one = pow(M_E, -(1.0 / 8.0) * (pow(x.at(i), 2.0) + pow(x.at(i + 1), 2.0) + (0.5 * (x.at(i + 1) * x.at(i)))));
    value_two = sqrt(pow(x.at(i), 2.0) + pow(x.at(i + 1), 2.0) + (0.5 * (x.at(i) * x.at(i + 1))));
